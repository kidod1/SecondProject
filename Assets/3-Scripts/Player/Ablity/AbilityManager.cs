...
public class AbilityManager : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // PlayerAbilityManager의 OnAbilitiesChanged 이벤트 구독
        if (playerAbilityManager != null)
        {
            playerAbilityManager.OnAbilitiesChanged += RefreshAbilityDescriptions;
        }
        else
        {
...
public void ShowAbilitySelection()
{
    ...
    for (int i = 0; i < abilitiesToShow; i++)
    {
        ...
        // 현재 능력의 레벨을 가져옴
        int currentLevel = playerAbilityManager.GetCurrentLevel(ability.abilityName);

        // Ability 인스턴스를 복제하여 currentLevel 설정
        Ability abilityInstance = Instantiate(ability);
        abilityInstance.currentLevel = currentLevel;

        abilityNameTexts[i].text = abilityInstance.abilityName;
        abilityDescriptionTexts[i].text = abilityInstance.GetDescription();
        abilityIcons[i].sprite = abilityInstance.abilityIcon;
    ...
    /// <summary>
    /// 능력 변경 시 호출되는 메서드로, UI의 설명 텍스트를 업데이트합니다.
    /// </summary>
    private void RefreshAbilityDescriptions()
    {
        for (int i = 0; i < abilitiesToShow; i++)
        {
        ...
        // 시너지 능력 패널의 설명도 업데이트
        if (isSynergyAbilityActive && currentSynergyAbility != null)
        {
            if (synergyAbilityDescriptionText != null)
            {
                // 시너지 능력의 경우에도 현재 레벨을 설정하여 설명 업데이트
                int currentLevel = playerAbilityManager.GetCurrentLevel(currentSynergyAbility.abilityName);
                SynergyAbility synergyInstance = Instantiate(currentSynergyAbility);
                synergyInstance.currentLevel = currentLevel;

                synergyAbilityDescriptionText.text = synergyInstance.GetDescription();
            }
            else
            {
                Debug.LogError("AbilityManager: synergyAbilityDescriptionText가 null입니다.");
            }
        }
    }
}